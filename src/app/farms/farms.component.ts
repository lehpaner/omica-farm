...
constructor(private _appCtx: AppCtx, private _titleService: Title,
              ...
              private _dapi: DAPI, private _farmsService: FarmsService,
              private _changeDetectorRef: ChangeDetectorRef,
              public media: TdMediaService, private _router: Router) {
                this.currentUser = this._appCtx.getCurrentUser();
                this.currentWorkspace = this._appCtx.getConfig('workspace');
...
ngOnInit(): void {
    this._titleService.setTitle('Manage farms');
...
async load(): Promise<void> {
    ...
    try {
      ...
      if(this.currentUser) {
        response = await this._farmsService.getFarms();
      } else {
      ...
      if (response.result === 'SUCESS') { //'SUCESS'|'ERROR'|'PROGRESS'
        ...
        if(! Array.isArray(response.data) ){
            this.pratiche.push(response.data);
        } else {
...
goToEdit(id: string): void {
  ...
  if (this.isSospeso) {
    canDo = false;
    messaggio = 'User results in "PAUSED" and disabed to modufy farm';
  }
