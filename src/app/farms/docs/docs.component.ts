...
import { ModuloCommissari, IDocument, IEsitoVerifica, IResultRestModel, RestResponse } from '../../model';
...
export class DocsComponent implements OnInit {
  ...
  async load(id: string): Promise<void> {
        ...
        try {
            ...
            response = await this._iscrizioneService.getFarmsDocs(id);
        } catch (error) {
  ...
  async upload(file: File, tipo: string): Promise<any> {
    let resp: IResultRestModel = null;
    try {
    this._loadingService.register('allegati.form');
    resp = await this._iscrizioneService.uploadFarmDoc(this.id, tipo, file);
     ...
     if (resp.result === 'SUCESS' ) {//la risposta è lista dei documenti
        ...
        if (ss === 20) {
         ...
         let errorID: number = resp.errorNumber;
 ...
 async generaPdf(docId: string): Promise<any> {
    try {
      this._loadingService.register('allegati.form');
      let result = await this._iscrizioneService.generatePdf(docId);
    } catch (error) {
      console.log(`An error has occured : ${error}`);
      let alertConfig: IAlertConfig = {
 ...
 async checkFirma(): Promise<any> {
    ...
    try {
     this._loadingService.register('allegati.form');
     result = await this._iscrizioneService.vediFirma(this.id);
   } catch (error) {
     console.log(`An error has occured : ${error}`);
     let alertConfig: IAlertConfig = {
        title: 'Errore applicazionne',
 ...
 async downloadDoc(doc): Promise<void> {
    try {
     ...
     await this._iscrizioneService.getAllegato(doc);
   } catch (error) {
     console.log(`An error has occured : ${error}`);
     let alertConfig: IAlertConfig = {
        title: 'Errore applicazionne',
