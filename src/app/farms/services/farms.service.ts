...
import { Response, RequestOptionsArgs, Headers, ResponseContentType, RequestOptions } from '@angular/http';
import { AppCtx }       from '../../app.context';
import { DAPI } from '../../services/dapi';
import { ModuloCommissari, IEsitoVerifica, RestResponse, IResultRestModel } from '../../model';
...
import 'rxjs/Rx' ;
import { IQNode } from 'src/app/nodes/model';
...
export class FarmsService {
  ...
  private currentWorkspace: any = null;

  constructor(private _ctx: AppCtx,  private _dapi:DAPI, private _fileUploadService: TdFileService) {
    this.currentWorkspace = _ctx.getConfig('workspace');
  ...
  public getFarm(id: string): Promise<IResultRestModel> {
    return this._dapi.nodeGeByIDAsync(id);
  }

  public getFarmsDocs(id: string): Promise<IResultRestModel> {
    return this._dapi.nodeGetChildrenOfType(id, 2003);
  }

  public uploadFarmDoc(id: string, tipo: string, file): Promise<IResultRestModel> {
    return this._dapi.nodeGetChildrenOfType(id, 2003);
  }

  public generatePdf(id: string): Promise<IResultRestModel> {
    return this._dapi.nodeGetChildrenOfType(id, 2003);
  }

  public vediFirma(id: string): Promise<IResultRestModel> {
    return this._dapi.nodeGetChildrenOfType(id, 2003);
  }
  public getAllegato(id: string): Promise<IResultRestModel> {
    return this._dapi.nodeGetChildrenOfType(id, 2003);
  }
  public saveFarm(data: IQNode): Promise<IResultRestModel> {
    return this._dapi.nodeCreateAsync(data);
  }
  public updateFarm(data: IQNode): Promise<IResultRestModel> {
    return this._dapi.nodeUpdateAsync(data);
  }
