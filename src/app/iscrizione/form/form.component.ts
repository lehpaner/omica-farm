...
import { Profile, ModuloCommissari, DatiIniziali, SezoniSottosezioni, RestResponse} from '../../model';
...
//              this.user = this._appCtx.getCurrentUser();
    ...
    async load(id: string): Promise<void> {
        ...
        try {
            if (id === 'new') { return; }
            response = await this._iscrizioneService.get(id).toPromise();
            if (response.errorID === 0) {
                this.currentData = response.dato;
    ...
    async save(): Promise<void> {
        ...
        try {
            ...
            if (this.action === 'add') {
                response = await this._iscrizioneService.nuovoModulo(this.currentData).toPromise();
            } else {
                response = await this._iscrizioneService.salvaModulo(this.currentData.id, this.currentData).toPromise();
            }
